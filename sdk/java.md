...
## [6.2.0] - 2026-05-18
- Thrown exceptions now use the API's error message when available, falling back to the canned message otherwise.

## [6.1.0] - 2026-05-18
- us-enrichment-api
  - **Breaking:** Removed the risk dataset (`RiskLookup`, `RiskResponse`, `RiskAttributes`, the `riskDataSet` constant, and the `sendRisk` client method). The corresponding endpoint has been removed from the API.
...
- Added an error when `withSender()` is combined with `withMaxTimeout()` or `withProxy()`, as these options only apply to the built-in HTTP transport.
...
## [5.3.3] - 2026-03-11
- Added proguard rules for Android.
...
- us-street-api
  ...
  - Added IANA timezone example and integration test.
...
## [4.1.3] - 2025-07-10
- Added better error reporting for miscellaneous response codes.
