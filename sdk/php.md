...
- Added `Source` class to `US_Autocomplete_Pro` and `US_Reverse_Geo` for typed source constants (`Source::All`, `Source::Postal`).
...
## [7.1.1] - 2026-06-08
- Updated PHP and PHPUnit version and associated test units.
...
- us-enrichment-api
  ...
  - Added ETag support across enrichment lookups, including `RequestNotModifiedException` handling for 304 responses.
...
- Added an error when `withSender()` is combined with `withMaxTimeout()`, `viaProxy()`, or `withDebug()`, as these options only apply to the built-in HTTP transport.

## [6.3.0] - 2026-03-12
- international-autocomplete-api
  - Added `geolocation` and `max_group_results` fields to Lookup.
...
- us-street-api
  ...
  - Added `withFeatureIanaTimeZone()` to ClientBuilder to enable the IANA timezone feature.
