...
## [13.2.0] - 2026-05-14
- us-enrichment-api
  - **Breaking:** Removed the risk dataset (`Risk.Lookup`, `Risk.Result`, `Risk.Attributes`, and the `SendRiskLookup`/`SendRiskLookupAsync` client methods). The corresponding endpoint has been removed from the API.
...
- Added an error when `WithSender()` is combined with `WithMaxTimeout()` or `ViaProxy()`, as these options only apply to the built-in HTTP transport.

## [12.3.0] - 2026-03-12
- international-autocomplete-api
  - Added `MaxGroupResults` field to Lookup (default: 100) to control the maximum number of grouped results returned.
  - Added `Geolocation` boolean field to Lookup. When set to `true`, sends `geolocation=on` to enable geolocation in results.

## [12.2.2] - 2026-03-10
- international-street-api
  - Fixed the minimum lookup requirements. Only country and either freeform or address1 are required.
...
## [12.0.1] - 2026-01-23
- us-street-api
  - Ambiguous matches are set to 5 for enhanced match when NOT set explicitly.

## [12.0.0] - 2026-01-23
- Added support for Basic Auth
- us-street-api
  - Default match is now set to Enhanced if not explicitly set.
  - Ambiguous matches are set to 5.
