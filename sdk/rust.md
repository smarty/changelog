...
## [0.15.1] - 2026-03-27
- General retry function introduced to client macro

## [0.15.0] - 2026-03-12
- international-autocomplete-api
    - Added `max_group_results` field to Lookup (default: 100) for controlling the maximum number of grouped results.
    - Added `geolocation` field to Lookup to enable geolocation in autocomplete requests.

## [0.14.2] - 2026-03-10
- international-street-api
    - Added validation to require `country` and either `freeform` or `address1` fields before sending a request.

## [0.14.1] - 2026-03-09
- us-street-api
    - Always send `match` parameter in requests, including `match=strict`.
    - Apply `candidates=5` default for enhanced match in both query string (single) and JSON (batch) requests.
    - Fixed `OutputFormat` serialization in batch JSON to use correct API values instead of Rust variant names.
    - Added integration tests for single/batch lookups with enhanced, strict, and mixed match strategies.

## [0.14.0] - 2026-03-04
- us-street-api
    - Added three new fields to the Metadata struct: `iana_time_zone` (string), `iana_utc_offset` (f32), and `iana_dst` (bool).
    - Added `with_iana_time_zone` option to enable the `iana-timezone` feature flag.
...
## [0.12.0] - 2026-01-29
- us-enrichment-api
    - Added support for address search
