...
## [11.2.0] - 2026-07-08
- us-autocomplete-api
  - Added `urbanization` property to `USAutocompleteSuggestion`, for Puerto Rico addresses.
...
## [10.1.1] - 2026-05-29
- international-street-api
  - Made LanguageMode init public.

## [10.1.0] - 2026-05-18
- us-enrichment-api
  - **Breaking:** Removed the risk dataset (`RiskEnrichmentLookup`, `RiskResult`, `RiskAttributes`, `RiskSerializer`, and the `sendRiskLookup` client methods). The corresponding endpoint has been removed from the API.
...
## [9.4.0] - 2026-03-26
- Added `withSender()` option to `ClientBuilder`, allowing a custom HTTP sender to be used as the innermost transport while keeping the full middleware chain intact (signing, retries, status codes, headers, URL prefix, license).
- Added an error when `withSender()` is combined with `withMaxTimeout()`, `withProxy()`, or `withDebug()`, as these options only apply to the built-in HTTP transport.

## [9.3.0] - 2026-03-12
- international-autocomplete-api
  - Added `max_group_results` field to lookup (default 100).
  - Added `geolocation` field to lookup (boolean, sends "on" when true).

## [9.2.2] - 2026-03-10
- international-street-api
  - Fixed the minimum lookup requirements.

## [9.2.1] - 2026-03-09
- us-street-api
  - Always send `match` parameter in requests (previously omitted for strict).
  - Apply `match` and `candidates` defaults consistently in both single (query string) and batch (JSON) requests.

## [9.2.0] - 2026-03-04
- us-street-api
  - Added IANA timezone fields (iana_time_zone, iana_utc_offset, iana_dst) to metadata response, accessible via the "iana-timezone" feature flag.
